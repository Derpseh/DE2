...
#initial dialog for grabbing a nation name to shove into the user agent. Setting headers while I'm at it.
    def showDialog(self):
        ...
        if ok:
            ...
            try:
                testReq = urllib2.Request('https://www.nationstates.net/cgi-bin/api.cgi?nation=' + UAgent.replace(' ', '_') + "&q=influence", None, headers)
    ...
    def exitDialog(self):
        exitBox = QtGui.QMessageBox()
        exitBox.setIcon(QtGui.QMessageBox.Warning)
        exitBox.setWindowTitle("Error")
        exitBox.setText("Nation not found. Please be sure to input the name of a nation that actually exists.")
        exitBox.exec_()
        quit()
